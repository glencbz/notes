...
MongoDB installs with a client app, a JavaScript-based shell, that allows us to interact with MongoDB directly. We can use it by running the `mongo` command in our terminal.
...
6. Lets switch to the `local` database: `> use local`.

7. Show the collections of the current database `> show collections`.
...
Using a collection for the first time creates it! We should get this:
```bash
